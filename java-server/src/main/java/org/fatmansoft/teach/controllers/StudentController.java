...
import org.fatmansoft.teach.util.PDFUtil;
...
public class StudentController {
    ...
    public DataResponse studentDelete(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if (s != null) {
            ...
            absenceRepository.deleteByStudent(s);
            dailyActivityRepository.deleteByStudent(s);
            honorRepository.deleteByStudent(s);
            innovationRepository.deleteByStudent(s);
    ...
    public DataResponse studentEditSave(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if (s == null) {
            ...
            s.setPerson(p);
            Student student = studentRepository.saveAndFlush(s);//插入新的Student记录
            Innovation innovation = new Innovation();
            List<Innovation> inns = innovationRepository.findByStudentId(student.getStudentId());
            ...
            if (inns.size()>1){
                throw new ServiceException("已存在该学生的创新实践记录");
            }
            isNew = true;
        } else {
            p = s.getPerson();
    ...
    public String importFeeData(Integer studentId,InputStream in){
        ...
        while (rowIterator.hasNext()) {
            ...
            if (cell == null)
                break;
            day = new DataFormatter().formatCellValue(cell);  //获取一行消费记录 日期 金额
            cell = row.getCell(1);
            money =  new DataFormatter().formatCellValue(cell);
    ...
    public byte[] getStudentIntroduceItextPdfData(Integer studentId,String barBase64, String lineBase64, String pieBase64) {
        ...
        try {
            ...
            //            获取个人头像
            File file = new File(attachFolder + student.getPerson().getPersonId() + ".jpg");
            int len = (int) file.length();
            byte d[] = new byte[len];
            FileInputStream in = new FileInputStream(file);
            in.read(d);
            in.close();
            String s = new String(Base64.getEncoder().encode(d));
            System.out.println("s = " + s);
            Element element = PDFUtil.analysisPicBase64Info(s);
            Image image = null;
...
//                image.setAbsolutePosition(0,  20+doc.getPageSize().getTop());
                System.out.println("image = " + image);
            }
            //新建单元格
            //给单元格赋值 每个单元格为一个段落，每个段落的字体为加粗
            PdfPCell cell11 = new PdfPCell(new Paragraph("姓名：  " + person.getName(), boldFont));
            PdfPCell cell12 = new PdfPCell(new Paragraph("性别：  " +("1".equals(person.getGender())?"男":"女"), boldFont));
            ...
            PdfPCell cell15 = new PdfPCell(new Paragraph(String.format("学号: %s",userId), boldFont));
            PdfPCell cell16 = new PdfPCell(new Paragraph(String.format("出生日期: %s",person.getBirthday()), boldFont));
            cell15.setBorder(0);
            cell16.setBorder(0);
            table1.addCell(cell15);
            table1.addCell(cell16);
            PdfPCell cell7 = new PdfPCell(new Paragraph(String.format("班级: %s",student.getClassName()),boldFont));
            PdfPCell cell8 = new PdfPCell(new Paragraph(String.format("专业: %s",student.getMajor()),boldFont));
            cell7.setBorder(0);
            cell8.setBorder(0);
            table1.addCell(cell7);
            table1.addCell(cell8);
            PdfPCell cell9 = new PdfPCell(new Paragraph(String.format("学院: %s",person.getDept()),boldFont));
            PdfPCell cell10 = new PdfPCell(new Paragraph(String.format("证件: %s",person.getCard()),boldFont));
            cell9.setBorder(0);
            cell10.setBorder(0);
            table1.addCell(cell9);
            table1.addCell(cell10);
            PdfPCell cell13 = new PdfPCell(new Paragraph(String.format("电话: %s",person.getPhone()), boldFont));
            PdfPCell cell14 = new PdfPCell(new Paragraph(String.format("邮箱: %s",person.getEmail()), boldFont));
            PdfPCell cell17 = new PdfPCell(new Paragraph(String.format("地址: %s",person.getAddress()), boldFont));
            cell13.setBorder(0);
            cell14.setBorder(0);
            cell17.setBorder(0);
            ...
            doc.add(table1);
            doc.add(this.getStudentCourseInfo(boldFont));
            doc.add(this.getEcharts(barBase64,lineBase64,pieBase64,boldFont));
            doc.add(this.getStudentScore(student,boldFont));
            doc.close();
    ...
    private PdfPTable getStudentCourseInfo (Font font) {
        PdfPTable table = new PdfPTable(5);
        PdfPCell courseNoCell = new PdfPCell(new Paragraph("课程号", font));
        PdfPCell courseNameCell = new PdfPCell(new Paragraph("课程名", font));
        PdfPCell creditCell = new PdfPCell(new Paragraph("学分", font));
        PdfPCell scoreCell = new PdfPCell(new Paragraph("成绩", font));
        PdfPCell rankingCell = new PdfPCell(new Paragraph("排名", font));
        table.addCell(courseNoCell);
        table.addCell(courseNameCell);
        table.addCell(creditCell);
        table.addCell(scoreCell);
        table.addCell(rankingCell);
        Integer userId = CommonMethod.getUserId();
        Person person = userRepository.findById(userId).get().getPerson();
        ...
        for (Score score : byStudentStudentId) {
            courseNoCell = new PdfPCell(new Paragraph(score.getCourse().getNum().toString(), font));
            courseNameCell = new PdfPCell(new Paragraph(score.getCourse().getName(), font));
            creditCell = new PdfPCell(new Paragraph(score.getCourse().getCredit().toString(), font));
            scoreCell = new PdfPCell(new Paragraph(score.getMark()==null?"":score.getMark().toString(), font));
            rankingCell = new PdfPCell(new Paragraph(score.getRanking()==null?"":score.getRanking().toString(), font));
            table.addCell(courseNoCell);
            table.addCell(courseNameCell);
            table.addCell(creditCell);
            table.addCell(scoreCell);
    ...
    private PdfPTable getEcharts(String barBase64, String lineBase64, String pieBase64, Font font) {
        PdfPTable table = new PdfPTable(3);
        Element element = PDFUtil.analysisPicBase64Info(barBase64);
        Image image1 = null;
        if (element instanceof Image){
            image1 = (Image) element;
        }
        PdfPCell pdfPCell = new PdfPCell(image1, true);
        Element element2 = PDFUtil.analysisPicBase64Info(lineBase64);
        Image image2 = null;
        if (element instanceof Image){
            image2 = (Image) element2;
        }
        PdfPCell pdfPCell2 = new PdfPCell(image2, true);
        Element element3 = PDFUtil.analysisPicBase64Info(pieBase64);
        Image image3 = null;
        if (element instanceof Image){
            image3 = (Image) element3;
        }
        PdfPCell pdfPCell3 = new PdfPCell(image3, true);
    ...
    public PdfPTable getStudentScore(Student student,Font font){
        PdfPTable table = new PdfPTable(4);

        StudentStatistics byStudent = studentService.statistics();
        PdfPCell pdfPCell = new PdfPCell(new Paragraph("平均分", font));
        PdfPCell pdfPCell2 = new PdfPCell(new Paragraph("总课程", font));
        PdfPCell pdfPCell3 = new PdfPCell(new Paragraph("总学分", font));
        PdfPCell pdfPCell4 = new PdfPCell(new Paragraph("gpa", font));
        table.addCell(pdfPCell);
        table.addCell(pdfPCell2);
        table.addCell(pdfPCell3);
        table.addCell(pdfPCell4);
        PdfPCell cell5 = new PdfPCell(new Paragraph(byStudent.getAvgScore().toString(), font));
        PdfPCell cell6 = new PdfPCell(new Paragraph(byStudent.getCourseCount().toString(), font));
        PdfPCell cell7 = new PdfPCell(new Paragraph(byStudent.getCreditTotal().toString(), font));
        PdfPCell cell8 = new PdfPCell(new Paragraph(byStudent.getGpa().toString(), font));
        table.addCell(cell5);
        table.addCell(cell6);
        table.addCell(cell7);
        table.addCell(cell8);
    ...
    public ResponseEntity<StreamingResponseBody> getStudentIntroduceItextPdf(@Valid @RequestBody DataRequest dataRequest) {
        Integer studentId = dataRequest.getInteger("studentId");
        String barBase64 = dataRequest.getString("barBase64");
        String lineBase64 = dataRequest.getString("lineBase64");
        String pieBase64 = dataRequest.getString("pieBase64");
        return CommonMethod.getByteDataResponseBodyPdf(getStudentIntroduceItextPdfData(studentId, barBase64, lineBase64, pieBase64));
    }
    ...
    @GetMapping("/honorByStudent/{studentId}")
    public Honor getHonorByStudent(@PathVariable Integer studentId) {
        honorRepository.getHonorByStudent(studentId);
        return null;
    }
    ...
    @PostMapping("/exportPersonIntro")
    public void exportPersonImage( HttpServletResponse response) throws IOException, DocumentException {
        Document document = new Document(PageSize.A4,Float.parseFloat("0"),Float.parseFloat("5"),Float.parseFloat("34"),Float.parseFloat("50"));

//        Paragraph test = PDFUtil.createImageFromEncodeBase64(echarts.getPieBase64(), null, Float.parseFloat("100"), Float.parseFloat("100"), false);
//        ArrayList<String> strings = new ArrayList<>();
//        strings.add(echarts.getBarBase64());
//        strings.add(echarts.getLineBase64());
//        ArrayList<String> titles = new ArrayList<>();
//        titles.add("日常消费");
//        titles.add(null);
//        ArrayList<Float> percentXs = new ArrayList<>();
//        percentXs.add(Float.parseFloat("150"));
//        percentXs.add(Float.parseFloat("150"));
//        ArrayList<Float> percentYs = new ArrayList<>();
//        percentYs.add(Float.parseFloat("100"));
//        percentYs.add(Float.parseFloat("100"));
//        Paragraph imageFromEncodeBase64Batch = PDFUtil.createImageFromEncodeBase64_batch(strings, null, percentXs, percentYs, false);
//
        Integer userId = CommonMethod.getUserId();
        Person person = userRepository.findById(userId).get().getPerson();
        Student student = studentRepository.getStudentByPerson(person);
        List<Innovation> innovation = innovationRepository.findByStudentId(student.getStudentId());
        ...
        Paragraph honorTitile = new Paragraph("荣誉", FontChinese24);
        paragraphs.add(honorTitile);
        List<Honor> honorStudent = honorRepository.findByStudent(student);
        PdfPTable table = new PdfPTable(4);
        table.addCell(new PdfPCell(new Paragraph("荣誉名称", boldFont)));
        table.addCell(new PdfPCell(new Paragraph("获得时间", boldFont)));
        table.addCell(new PdfPCell(new Paragraph("描述", boldFont)));
        table.addCell(new PdfPCell(new Paragraph("奖励", boldFont)));
        for (Honor honor : honorStudent) {
            table.addCell(new PdfPCell(new Paragraph(honor.getName(), FontChinese12)));
            table.addCell(new PdfPCell(new Paragraph(honor.getGetTime(), FontChinese12)));
            table.addCell(new PdfPCell(new Paragraph(honor.getDescription(), FontChinese12)));
            table.addCell(new PdfPCell(new Paragraph(honor.getReward(), FontChinese12)));
        }
        Paragraph elements = new Paragraph();
        elements.add(table);
        paragraphs.add(elements);
        Innovation innoStudent = innovationRepository.findByStudentId(student.getStudentId()).get(0);
        Paragraph innoTitle = new Paragraph("创新", FontChinese24);
        paragraphs.add(innoTitle);
        Paragraph inno = new Paragraph(String.format("进行了%s等社会实践，参加了%s等学科竞赛，拥有%s等科技成果，参加了%s等培训讲座，进行了%s等创新项目，参加了%s等校外实习", innoStudent.getSocialPractice(), innoStudent.getCompetition(), innoStudent.getTechnology(), innoStudent.getLecture(), innoStudent.getProject(), innoStudent.getInternship()), FontChinese12);
        paragraphs.add(inno);

//        paragraphs.add(test);
//        paragraphs.add(imageFromEncodeBase64Batch);
        PDFUtil.exportDocument(doc, null,paragraphs,response,"报表");
    }
