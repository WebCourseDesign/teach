 ...
 * 根据关联的Student的student_id查询获得List<Score>对象集合, 命名规范
 */
...
public interface ScoreRepository extends JpaRepository<Score, Integer> {
    ...
    @Query(value = "select * from score where course_id in (?1)", nativeQuery = true)
    List<Score> findByCourses(List<Course> course);
